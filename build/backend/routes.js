...
router.post('/reset-password', controllers.auth.resetPassword); // router.post('/session/:id', auth.verifySession)

router.get('/users', auth.checkAdmin, controllers.users.index);
router.get('/users/:id', auth.idMatchesOrAdmin, controllers.users.get);
router.get('/users/admin/:id', auth.makeAdmin, controllers.users.get); // more complex permissions checking (need admin to create admin) done in function
